...
from .utils import generate_certificate_docx


class CertificateViewSet(viewsets.ModelViewSet):
    queryset = Certificate.objects.select_related('template', 'specialization', 'created_by').all()
    ...
    def _generate_doc(self, instance):
        try:
            ...
            docx_file = generate_certificate_docx(instance, verification_url)
            instance.generated_pdf.save(docx_file.name, docx_file, save=True)
        except Exception as e:
            print(f"Doc generation warning: {e}")
    ...
    def regenerate(self, request, pk=None):
        ...
        try:
            ...
            docx_file = generate_certificate_docx(certificate, verification_url)
            certificate.generated_pdf.save(docx_file.name, docx_file, save=True)
            return Response({
                'message': 'Muvaffaqiyatli qayta yaratildi',
    ...
    def calculate_end_date(self, request):
        ...
        if not start_date_str or not duration:
            return Response({'error': 'start_date va duration_days kerak'}, status=400)
        try:
